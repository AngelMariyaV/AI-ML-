...
Original file is located at
    https://colab.research.google.com/drive/1TvonuHmy7zCEGuQPQBlaPlRyUQO37h3c
"""
...
def draw_board(board, scores):
    ...
    screen_x = OFFSET_X
    stdscr.addstr(screen_y, screen_x, "Player 1: %d" % player1, curses.color_pair(1))
    stdscr.addstr(screen_y + 1, screen_x, "Player 2: %d" % player2, curses.color_pair(2))
...
def play_game():
    ...
    def check_and_set_square(x, y):
        if score[int(x)][int(y)]:
            return 0
        if (board[int(x * 2)][int(y)] and          # above
            board[int(x * 2 + 1)][int(y)] and      # left
            board[int(x * 2 + 1)][int(y + 1)] and  # right
            board[int(x * 2 + 2)][int(y)]):        # bottom
                score[int(x)][int(y)] = current_player
                return 1
        return 0
